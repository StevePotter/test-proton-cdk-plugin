...
And that's it: now you can head over to http://localhost:3000/ to hit `StartProvisioningFunction`!
...
```bash
$ sam local invoke StartProvisioningFunction --env-vars env.json --event events/event-example.json
```
...
You can debug with external debugger by this manual: [Step-through debugging Node.js functions locally ](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-using-debugging-nodejs.html)
 ...
 1. Run `sam local invoke` with `--debug-port` option.
    ...
    ```sh
    $ sam local invoke StartProvisioningFunction --env-vars=env.json --event events/event-example.json --debug-port 5858
    ```
...
```bash
$ sam logs -n StartProvisioningFunction --stack-name sam-app --tail
```
